...
} from "./chunk-XDPP4PZC.js";
import {
  firebase
} from "./chunk-NMDHFU2A.js";
import {
  $l,
  Aa,
  Bl,
...
async function attemptFetchDynamicConfigWithRetry(appFields, { throttleEndTimeMillis, backoffCount }, signal, retryData = defaultRetryData) {
  ...
  try {
    ...
    if (!isRetriableError(error2)) {
      retryData.deleteThrottleMetadata(appId);
      if (measurementId) {
        logger.warn(`Failed to fetch this Firebase app's measurement ID from the server. Falling back to the measurement ID ${measurementId} provided in the "measurementId" field in the local Firebase config. [${error2 === null || error2 === void 0 ? void 0 : error2.message}]`);
        return { appId, measurementId };
      } else {
        throw e;
      }
...
var name3 = "@firebase/analytics-compat";
var version3 = "0.2.0";
...
registerAnalytics2();
firebase.registerVersion(name3, version3);
...
var ERROR_FACTORY3 = new ErrorFactory("appCheck", "AppCheck", ERRORS2);
...
function factory3(app, heartbeatServiceProvider) {
...
var ReCaptchaV3Provider = class {
  ...
  async getToken() {
    ...
    const attestedClaimsToken = await getToken$1(this._app).catch((_e) => {
      throw ERROR_FACTORY3.create("recaptcha-error");
    });
    let result;
    try {
      ...
      if ((_a = e.code) === null || _a === void 0 ? void 0 : _a.includes("fetch-status-error")) {
        ...
        throw ERROR_FACTORY3.create("throttled", {
...
function registerAppCheck() {
  _registerComponent(new Component(APP_CHECK_NAME, (container) => {
    ...
    return factory3(app, heartbeatServiceProvider);
...
var name5 = "@firebase/app-check-compat";
var version5 = "0.3.0";
var ERRORS3 = {
...
registerAppCheck2();
firebase.registerVersion(name5, version5);
...
var RepoInfo = class {
  constructor(host, secure, namespace, webSocketOnly, nodeAdmin = false, persistenceKey2 = "", includeNamespaceInQueryParams = false) {
    this.secure = secure;
...
function repoSendReadyTransactions(repo, node = repo.transactionQueueTree_) {
  ...
  if (treeGetValue(node)) {
    ...
    const allRun = queue2.every((transaction) => transaction.status === 0);
    if (allRun) {
...
function remove2(ref3) {
  validateWritablePath("remove", ref3._path);
  return set2(ref3, null);
}
function set2(ref3, value) {
...
var Reference = class extends Query {
  ...
  set(newVal, onComplete) {
    ...
    const result = set2(this._delegate, newVal);
    if (onComplete) {
      result.then(() => onComplete(null), (error2) => onComplete(error2));
    }
    return result;
  ...
  update(values, onComplete) {
    ...
    const result = update2(this._delegate, values);
    if (onComplete) {
      result.then(() => onComplete(null), (error2) => onComplete(error2));
    }
    return result;
  ...
  remove(onComplete) {
    ...
    const result = remove2(this._delegate);
    if (onComplete) {
      result.then(() => onComplete(null), (error2) => onComplete(error2));
    }
    return result;
...
var ERROR_FACTORY5 = new ErrorFactory("messaging", "Messaging", ERROR_MAP);
...
async function registerDefaultSw(messaging) {
  try {
    ...
    throw ERROR_FACTORY5.create("failed-service-worker-registration", {
...
async function updateSwReg(messaging, swRegistration) {
  ...
  if (!(swRegistration instanceof ServiceWorkerRegistration)) {
    throw ERROR_FACTORY5.create("invalid-sw-registration");
  }
...
async function getToken$12(messaging, options) {
  if (!navigator) {
    throw ERROR_FACTORY5.create("only-available-in-window");
  }
  if (Notification.permission === "default") {
    await Notification.requestPermission();
  }
  if (Notification.permission !== "granted") {
    throw ERROR_FACTORY5.create("permission-blocked");
  }
...
var ERROR_FACTORY6 = new ErrorFactory("messaging", "Messaging", ERROR_MAP2);
...
function onBackgroundMessage$1(messaging, nextOrObserver) {
  if (self.document !== void 0) {
    throw ERROR_FACTORY6.create("only-available-in-sw");
  }
...
function missingPolyFill(polyFill) {
  return new StorageError("unsupported-environment", `${polyFill} is missing. Make sure to install the required polyfills. See https://firebase.google.com/docs/web/environments-js-sdk#polyfills for more information.`);
}
...
var ERROR_FACTORY7 = new ErrorFactory(SERVICE2, SERVICE_NAME2, ERROR_DESCRIPTION_MAP2);
...
var Api = class {
  constructor(window2) {
    this.window = window2;
    if (!window2) {
      throw ERROR_FACTORY7.create("no window");
    }
...
function mergeStrings(part1, part2) {
  ...
  if (sizeDiff < 0 || sizeDiff > 1) {
    throw ERROR_FACTORY7.create("invalid String merger input");
  }
  const resultArray = [];
...
function getAppId(firebaseApp) {
  ...
  if (!appId) {
    throw ERROR_FACTORY7.create("no app id");
  }
  return appId;
}
function getProjectId(firebaseApp) {
  ...
  if (!projectId) {
    throw ERROR_FACTORY7.create("no project id");
  }
  return projectId;
}
function getApiKey(firebaseApp) {
  ...
  if (!apiKey) {
    throw ERROR_FACTORY7.create("no api key");
  }
  return apiKey;
}
...
function getRemoteConfig(performanceController, iid2) {
  return getAuthTokenPromise(performanceController.installations).then((authToken) => {
    ...
    return fetch(request).then((response) => {
      ...
      throw ERROR_FACTORY7.create("RC response not ok");
    });
  }).catch(() => {
...
function addToQueue(evt) {
  if (!evt.eventTime || !evt.message) {
    throw ERROR_FACTORY7.create("invalid cc log");
  }
  queue = [...queue, evt];
}
...
var Trace = class {
  ...
  start() {
    if (this.state !== 1) {
      throw ERROR_FACTORY7.create("trace started", {
        traceName: this.name
      });
    }
  ...
  stop() {
    if (this.state !== 2) {
      throw ERROR_FACTORY7.create("trace stopped", {
        traceName: this.name
      });
    }
    this.state = 3;
  ...
  record(startTime, duration, options) {
    if (startTime <= 0) {
      throw ERROR_FACTORY7.create("nonpositive trace startTime", {
        traceName: this.name
      });
    }
    if (duration <= 0) {
      throw ERROR_FACTORY7.create("nonpositive trace duration", {
        traceName: this.name
      });
    }
  ...
  putMetric(counter, numAsInteger) {
    if (isValidMetricName(counter, this.name)) {
      ...
      throw ERROR_FACTORY7.create("invalid custom metric name", {
  ...
  putAttribute(attr, value) {
    ...
    if (!isValidName) {
      throw ERROR_FACTORY7.create("invalid attribute name", {
        attributeName: attr
      });
    }
    if (!isValidValue) {
      throw ERROR_FACTORY7.create("invalid attribute value", {
...
var factory8 = (container, { options: settings2 }) => {
  ...
  if (app.name !== DEFAULT_ENTRY_NAME) {
    throw ERROR_FACTORY7.create("FB not default");
  }
  if (typeof window === "undefined") {
    throw ERROR_FACTORY7.create("no window");
  }
...
var ERROR_FACTORY8 = new ErrorFactory("remoteconfig", "Remote Config", ERROR_DESCRIPTION_MAP3);
...
var RestClient = class {
  ...
  async fetch(request) {
    ...
    try {
      ...
      throw ERROR_FACTORY8.create(errorCode, {
        originalErrorMessage: originalError === null || originalError === void 0 ? void 0 : originalError.message
      });
    }
    ...
    if (response.status === 200) {
      ...
      try {
        ...
        throw ERROR_FACTORY8.create("fetch-client-parse", {
          originalErrorMessage: originalError === null || originalError === void 0 ? void 0 : originalError.message
        });
      }
    ...
    if (status !== 304 && status !== 200) {
      throw ERROR_FACTORY8.create("fetch-status", {
        httpStatus: status
      });
    }
...
function setAbortableTimeout2(signal, throttleEndTimeMillis) {
  return new Promise((resolve, reject) => {
    ...
    signal.addEventListener(() => {
      clearTimeout(timeout);
      reject(ERROR_FACTORY8.create("fetch-throttle", {
        throttleEndTimeMillis
      }));
    });
  });
...
function toFirebaseError(event, errorCode) {
  ...
  return ERROR_FACTORY8.create(errorCode, {
...
function openDatabase() {
  return new Promise((resolve, reject) => {
    try {
      ...
      reject(ERROR_FACTORY8.create("storage-open", {
  ...
  async get(key) {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        reject(ERROR_FACTORY8.create("storage-get", {
          originalErrorMessage: e === null || e === void 0 ? void 0 : e.message
        }));
      }
    });
  ...
  async set(key, value) {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        reject(ERROR_FACTORY8.create("storage-set", {
          originalErrorMessage: e === null || e === void 0 ? void 0 : e.message
        }));
      }
    });
  ...
  async delete(key) {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        reject(ERROR_FACTORY8.create("storage-delete", {
          originalErrorMessage: e === null || e === void 0 ? void 0 : e.message
        }));
      }
    });
...
function registerRemoteConfig() {
  ...
  function remoteConfigFactory2(container, { instanceIdentifier: namespace }) {
    ...
    const installations = container.getProvider("installations-internal").getImmediate();
    if (typeof window === "undefined") {
      throw ERROR_FACTORY8.create("registration-window");
    }
    if (!isIndexedDBAvailable()) {
      throw ERROR_FACTORY8.create("indexed-db-unavailable");
    }
    const { projectId, apiKey, appId } = app.options;
    if (!projectId) {
      throw ERROR_FACTORY8.create("registration-project-id");
    }
    if (!apiKey) {
      throw ERROR_FACTORY8.create("registration-api-key");
    }
    if (!appId) {
      throw ERROR_FACTORY8.create("registration-app-id");
    }
...
var RemoteConfigCompatImpl = class {
  ...
  setLogLevel(logLevel) {
    setLogLevel2(this._delegate, logLevel);
  }
};
